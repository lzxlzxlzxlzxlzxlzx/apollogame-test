...
export const FLYER = 2, GROUND = 4, GROUND_WINDOW = 8, FLIGHT = 16, HARD_CONTROL = 32;
export const xf = (x: number) => ({ x, y: 0, rotation: 0, scaleX: 1, scaleY: 1 });
export function add(w: World, id: string, cs: Record<string, object>) { w.createEntity(id); for (const [type, value] of Object.entries(cs)) w.addComponent(id, { type, ...value } as Component); }
...
export function cancellableDive() {
  const w = world();
...
export function committedMelee() {
  const w = new World();

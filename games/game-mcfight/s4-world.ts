...
export interface CombatCatalog {
  ...
  templates: readonly (Omit<S4SkillTemplate,'id'|'form'> & { id:string; form:S4SkillTemplate['form']|'devour';maxTargetHp?:number;damageType?:'normal'|'true'; armorPiercing?:boolean; projectile?:boolean; fracOfMax?:number; volleyCount?:number; volleyIntervalTicks?:number; volleySpread?:{kind:'none'}|{kind:'seeded-uniform';halfAngleRadians:number}; shape?:'cone'; coneRadius?:number; coneAngleRadians?:number; pulseCount?:number; pulseIntervalTicks?:number; lockAimOnStart?:boolean; mobilityLockTicks?:number; mobilityLockArthropodOnly?:boolean; skillCycle?:boolean; cycleCount?:number; orbitRadius?:number; orbitSpeed?:number; formStage?:'base'|'changed'; formChange?:{threshold:number;formId:'changed';speed:number;airborne:boolean} })[];
}
...
export function buildCombatUnitTemplate(catalog:CombatCatalog,scope:string,unitId:string,team:1|2):PrefabTemplate {
  ...
  const cycleSequence=candidates.some(t=>t.skillCycle)?candidates.flatMap(t=>Array.from({length:t.cycleCount??1},()=>t.id)):undefined;
  ...
  if(cycleSequence?.length){
    (entities.body as any).SkillCycle={candidates:[...cycleSequence],index:0};
    entities.cycle={Hierarchy:parent(),State:{fsmId:ref('cycle'),current:cycleSequence[0]!,previous:cycleSequence[0]!}};
  }
  const form=candidates.find(t=>t.formChange)?.formChange;
  if(form){
    entities.form={Hierarchy:parent(),State:{fsmId:ref('form'),current:'base',previous:'base'}};
    (entities.body as any).FormChange={threshold:form.threshold,formId:form.formId,formStateEntity:ref('form'),formSpeed:form.speed,formAirborne:form.airborne,formPerceptionRejectStatusMask:0};
  }
  const orbit=candidates.find(t=>t.orbitRadius!==undefined);
  if(orbit) (entities.body as any).RelationOrbit={radius:orbit.orbitRadius!,speed:orbit.orbitSpeed??0,active:true};
  for (const skill of candidates) {
    ...
    const states: FlowState[] = [
      { id: 'Ready', transitions: [{ when: { kind: 'and', of: [
        ...
        ...(cycleSequence ? [{kind:'state' as const,fsmId:ref('cycle'),equals:skill.id}] : []),
        ...(skill.formStage ? [{kind:'state' as const,fsmId:ref('form'),equals:skill.formStage}] : []),
    ...
    entities[`${skill.id}-release`] = { Hierarchy: parent(), EventWhen: { signal: `${key}-release`, mode: 'edge', when: { kind: 'state', fsmId: key, equals: 'Active' } },
      Caster: { onSignal: `${key}-release`, template: `${scope}-${team}-${skill.id}`, at: target ? 'target' : 'self', originEntity: ref('body'),
        ...(cycleSequence ? {cycleStateEntity:ref('cycle'),cycleOwner:ref('body'),cycleCandidates:cycleSequence} : {}),
...
export function assembleCombatWorld(engine:Engine,catalog:CombatCatalog,scope:string):void {
  ...
  for (const team of [1, 2] as const) for (const skill of catalog.templates.filter(t => !t.blockedBy&&t.form!=='devour')) {
    templates[`${scope}-${team}-${skill.id}`] = { entities: { zone: {
      ...
      Hitbox: { resource: 'hp', ...(skill.fracOfMax === undefined ? { amount: skill.damage } : { fracOfMax: skill.fracOfMax }), targetMask: s4TeamMask(skill.form === 'heal' ? team : team === 1 ? 2 : 1), consumeOnHit: true, ...(skill.form === 'projectile' ? { singleImpact: true } : {}),
        ...(skill.onHitStatus ? { onHitStatus: [{ id: skill.onHitStatus, duration: skill.onHitStatus === 'burn' ? 200 : skill.onHitStatus === 'wither' ? 80 : 100, ...(skill.onHitStatus === 'burn' ? { damagePerTick: 1, period: 20 } : skill.onHitStatus === 'wither' ? { damagePerTick: 3, period: 20 } : skill.onHitStatus === 'poison' ? { damagePerTick: 2, period: 20 } : { multiplier: .7 }) }] } : {}),
        ...(skill.damageType?{damageType:skill.damageType}:{}),...(skill.armorPiercing?{armorPiercing:true}:{}),
        ...(skill.mobilityLockTicks ? { onHitMobilityLock:{duration:skill.mobilityLockTicks,groundWhileLocked:true,...(skill.mobilityLockArthropodOnly?{requireTagMask:16}:{})} } : {}),

...
type IdentityCardPile = { type: 'IdentityCardPile'; deck: string[]; hand: string[]; discard: string[]; handLimit: number; phase: string; playPhase: string; shuffled?: boolean };
...
export const identityCardPlayCapability = defineCapability({
  ...
  components: {
    provides: {
      ...
      IdentityCardPile: defineComponent('IdentityCardPile', { deck: t.arr(t.str('cardId'), '牌库'), hand: t.arr(t.str('cardId'), '手牌'), discard: t.arr(t.str('cardId'), '弃牌'), handLimit: t.num('手牌上限'), phase: t.str('当前时序'), playPhase: t.str('允许出牌的时序'), shuffled: t.opt(t.bool('内部洗牌标记')) }, { category: 'config', describe: '身份牌 deck/hand/discard。' }),
    ...
    reads: ['CardCatalog', 'IdentityCardPile', 'IdentityCardCommand', 'RandomSeed', 'Resource', 'DebugTrace'], writes: ['IdentityCardPile', 'IdentityCardCommand', 'RandomSeed', 'ResourceModify'], consumes: [],
  }, config: {},
  systems: [{ id: 'identity-card-play', phase: SystemPhase.Intent, reads: ['CardCatalog', 'IdentityCardPile', 'IdentityCardCommand', 'RandomSeed', 'Resource', 'DebugTrace'], writes: ['IdentityCardPile', 'IdentityCardCommand', 'RandomSeed', 'ResourceModify'], consumes: [],
    execute(world) {
      ...
      if (!pile.shuffled) { if (!rng) { reject(world, '缺 RandomSeed，洗牌拒绝'); return; } shuffle(pile.deck, rng); pile.shuffled = true; }
      while (pile.hand.length < pile.handLimit) { if (!pile.deck.length && pile.discard.length) { if (!rng) { reject(world, '缺 RandomSeed，弃牌回收拒绝'); break; } pile.deck.push(...pile.discard.splice(0)); shuffle(pile.deck, rng); } if (!pile.deck.length) break; pile.hand.push(pile.deck.shift()!); }
      let reservedFocus = 0;

...
/** 每 tick 开始从 Transform 复制的只读位置；决策系统读它，碰撞系统继续读移动后的 Transform。 */
export interface FrameStartTransform extends Component {
  readonly type: 'FrameStartTransform';
  x: number;
  y: number;
}
...
export interface Shape extends Component {
  readonly type: 'Shape';
  kind: 'box' | 'circle' | 'polygon' | 'capsule' | 'cone';
  /** Capsule centerline length; endpoints are +/- length/2. Width = 2*radius.
   * axisX/Y optionally supply a world-space direction, overriding rotation. */
  length?: number;
  axisX?: number;
  axisY?: number;
  width?: number;
  ...
  vertices?: number[];
  /** Cone fan shape: local forward axis is +X; angle is full radians. Radius is shared with circles. */
  angle?: number;
